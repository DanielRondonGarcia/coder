...
# following files:
# - go.mod
# - dogfood/DanielRondonGarcia/Dockerfile
# - flake.nix
...
GO_VERSION_DOCKERFILE=$(grep -Eo 'ARG GO_VERSION=[0-9]+\.[0-9]+\.[0-9]+' ./dogfood/DanielRondonGarcia/Dockerfile | cut -d'=' -f2)
...
log "INFO : dogfood/DanielRondonGarcia/Dockerfile : $GO_VERSION_DOCKERFILE"
...
if [ "$GO_VERSION_GO_MOD" != "$GO_VERSION_DOCKERFILE" ]; then
	error "Go version mismatch between go.mod and dogfood/DanielRondonGarcia/Dockerfile:"
fi
